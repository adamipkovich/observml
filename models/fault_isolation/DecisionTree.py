...
class DecisionTreeModel:
    ...
    def fit(self, X, y=None):
        ...
        #cols = nX.columns
        nX = self._std_scaler.fit_transform(nX.values)
        ...
        self._figs['performance'].add_trace(go.Heatmap(z=np.transpose(cm), x=self._model.classes_, y=self._model.classes_), row=1, col=1)
        ...
        self._figs['importance'].add_trace(go.Bar(x=self.scheme.to_list(), y=self._model.feature_importances_, name='Feature Importance'), row=1, col=1)
...
if __name__ == "__main__":
    ...
    nX2 = pd.read_excel("./data/detect_test_1.xlsx")
    nX2 = data.rename(columns={"Output (S)": "target", "ds": "ds"})
    nX2["ds"] = pd.to_datetime(nX2["ds"])
    nX2.set_index("ds", inplace=True)
    y2 = dt.predict(nX2)
    dt.update_predict(y2)

    dt._figs["Ia_prediction"].show()
